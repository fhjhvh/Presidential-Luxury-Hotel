...
const STAT_ICONS     = ['🏨', '📐', '👨‍👩‍👧‍👦', '🎯'];
const STAT_NUMS      = ['50', '75-90', 'Family', 'Premium'];
const STATUS_CLASSES = ['available', 'available', 'reserved'];

const Floor6 = () => {
  const { t } = useTranslation();
  const areas    = t('floorPages.f6.areas',    { returnObjects: true });
  const features = t('floorPages.f6.features', { returnObjects: true });
  const stats    = t('floorPages.f6.stats',    { returnObjects: true });

  return (
    <div className="floor-page" style={{ '--flr-hero-img': 'url(https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=1920&auto=format&fit=crop&q=80)' }}>
      <BackButton />
      <section className="floor-hero">
        ...
        <div className="container">
          <motion.div className="floor-hero__content" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="floor-hero__badge">{t('floorPages.f6.heroBadge')}</div>
            <h1 className="floor-hero__title">{t('floorPages.f6.heroTitle')}</h1>
            <p className="floor-hero__description">{t('floorPages.f6.heroDesc')}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          <RevealOnScroll>
            <div className="floor-overview">
              <h2 className="section-title">{t('floorPages.f6.overviewTitle')}</h2>
          ...
          <RevealOnScroll delay={0.2}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f6.areasTitle')}</h3>
          ...
          <RevealOnScroll delay={0.3}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f6.featuresTitle')}</h3>
