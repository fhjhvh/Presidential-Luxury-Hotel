...
const STAT_ICONS = ['🏠', '👥', '🎯', '🏆'];
const STAT_NUMS  = ['80', '150+', 'Premium', 'Best'];

const Floor9 = () => {
  const { t } = useTranslation();
  const areas    = t('floorPages.f9.areas',    { returnObjects: true });
  const features = t('floorPages.f9.features', { returnObjects: true });
  const stats    = t('floorPages.f9.stats',    { returnObjects: true });

  return (
    <div className="floor-page" style={{ '--flr-hero-img': 'url(https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=1920&auto=format&fit=crop&q=80)' }}>
      <BackButton />
      <section className="floor-hero">
        ...
        <div className="container">
          <motion.div className="floor-hero__content" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="floor-hero__badge">{t('floorPages.f9.heroBadge')}</div>
            <h1 className="floor-hero__title">{t('floorPages.f9.heroTitle')}</h1>
            <p className="floor-hero__description">{t('floorPages.f9.heroDesc')}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          <RevealOnScroll>
            <div className="floor-overview">
              <h2 className="section-title">{t('floorPages.f9.overviewTitle')}</h2>
          ...
          <RevealOnScroll delay={0.2}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f9.areasTitle')}</h3>
                ...
                {Array.isArray(areas) && areas.map((area, index) => (
                  <motion.div key={index} className="room-card" whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300 }}>
                    ...
                    <div className="room-card__header">
                      ...
                      <p className="room-card__type">{area.capacity}</p>
          ...
          <RevealOnScroll delay={0.3}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f9.featuresTitle')}</h3>
