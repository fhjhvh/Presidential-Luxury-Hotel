...
const STAT_ICONS     = ['🚪', '📏', '⭐', '🛎️'];
const STAT_NUMS      = ['80', '35-50', '5', '24/7'];
const STATUS_CLASSES = ['available', 'available', 'reserved', 'available'];

const Floor4 = () => {
  const { t } = useTranslation();
  const areas    = t('floorPages.f4.areas',    { returnObjects: true });
  const features = t('floorPages.f4.features', { returnObjects: true });
  const stats    = t('floorPages.f4.stats',    { returnObjects: true });

  return (
    <div className="floor-page" style={{ '--flr-hero-img': 'url(https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1920&auto=format&fit=crop&q=80)' }}>
      <BackButton />
      <section className="floor-hero">
        ...
        <div className="container">
          <motion.div className="floor-hero__content" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="floor-hero__badge">{t('floorPages.f4.heroBadge')}</div>
            <h1 className="floor-hero__title">{t('floorPages.f4.heroTitle')}</h1>
            <p className="floor-hero__description">{t('floorPages.f4.heroDesc')}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          <RevealOnScroll>
            <div className="floor-overview">
              <h2 className="section-title">{t('floorPages.f4.overviewTitle')}</h2>
          ...
          <RevealOnScroll delay={0.2}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f4.areasTitle')}</h3>
          ...
          <RevealOnScroll delay={0.3}>
            <div className="floor-section">
              <h3 className="section-subtitle">{t('floorPages.f4.featuresTitle')}</h3>
